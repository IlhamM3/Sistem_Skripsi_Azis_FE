    ...
    <main >
        <div id="Carrosal" class="relative w-full">
            <div class="relative md:h-[520px] h-[500px] overflow-hidden">
                <div class="absolute inset-0 md:-top-24 -top-32 z-20 flex justify-center text-white">
                    ...
                    <h1 class="md:px-14 text-semibold md:text-xl text-xs lg:text-xl capitalize">
                        "Powered by the Dual-Gate Multi-Head Temporal Fusion Transformer (DGMH-TFT), this system delivers highly accurate 24-hour ahead solar power predictions. By leveraging historical PV output, past weather records, and meteorological forecasts, it provides actionable insights to maximize renewable energy efficiency and grid stability."
                    </h1>
                    </div>
                </div>
        ...
        <div class="bg-white md:rounded-t-[2.5rem] rounded-t-[2rem] -top-14 relative md:-top-8 z-20 " id="CobaAI">
                ...
                <div class="max-w-screen-md w-full relative -top-28 md:-top-20">
                    <form @submit.prevent="submitForm" enctype="multipart/form-data" 
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <input ref="fileInput" type="file" name="file" id="file" required
                                    class="block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 
                                            file:rounded-lg file:border-0 file:text-sm file:font-semibold
                                            file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100 
                                            border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:outline-none"/>
                            </div>

                        <div class="flex justify-end">
                            <button type="submit"
                                    class="cursor-pointer px-6 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold border border-blue-600
                                        shadow hover:bg-blue-400 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            Detection
                            </button>
                        </div>
                    </form>
            ...
            <div id="PopHasil" aria-hidden="true" class="hidden items-center justify-center relative -top-16 md:-top-12">
                    ...
                    <table class="w-full text-sm md:text-lg text-left rtl:text-right">
                      ...
                      <tbody class="">
                        ...
                        >
                          ...
                          <td class="md:px-4 md:py-2 px-2 py-1">{{ data.prediction }}</td>
        ...
        <div className="relative w-full max-w-sm">
            <div className="bg-white rounded-2xl shadow-lg">
                <div className="flex flex-col justify-center items-center relative ">
                    <div className="w-full h-full flex justify-center items-center relative bg-black/20 rounded-2xl pl-2">
                    <div ref="lottieContainerSoil" className="lottie-animation w-full md:w-96"></div>
                    </div>
                </div>
            </div>
        </div>
...
const images = ref(["image1.png", "image2.png", "image3.png", "image4.png", "image5.png"])
...
const submitForm = async () => {
  ...
  try {
    ...
    const response = await axios.post(import.meta.env.VITE_BASE_URL + "predict", formData)
    ...
    ImagePred.value = data.prediction_files
    ImageInter.value = data.interpretation_file
